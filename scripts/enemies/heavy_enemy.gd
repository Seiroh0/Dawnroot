...
func _build_visuals() -> void:
	...
	anim_sprite.play()
	anim_sprite.position = Vector2(0, -18)
	...
	collision_shape.position = Vector2(0, -16)
...
func _update_palette() -> void:
	if anim_sprite:
		anim_sprite.modulate = palette.get("primary", Color(0.5, 0.35, 0.25))
...
func _create_frames() -> SpriteFrames:
	...
	_add_animation(frames, "pulse", true, 6.0, _frame_paths("enemy_heavy_walk_", 4), Vector2i(36, 36))
	death_frames = _load_textures(_frame_paths("enemy_heavy_death_", 4), Vector2i(36, 36))
	return frames
...
func _load_texture(path: String, size: Vector2i) -> Texture2D:
	...
	return GameConstants.make_rect_texture(palette.get("primary", Color(0.5, 0.35, 0.25)), size)
